...
class MCMCSampler(nn.Module):
    ...
    def __init__(self, num_steps, lr, tau=0.01, lr_min_ratio=0.01, prior_solver='gaussian', prior_sigma_min=1e-2,
                 mc_algo='langevin', momentum=0.9, vae=None):
        super().__init__()
    ...
    def gradient(self, x, measurement, operator, task, return_loss=False):
        """
    ...
    def score_fn(self, x, x0hat, xt, operator, measurement, sigma, task):
        """
        ...
        Returns:
            Tuple containing:
                ...
                - Data-fitting loss.
        """
        data_fitting_grad, data_fitting_loss = self.gradient(x, measurement, operator, task, return_loss=True)
        data_term = -data_fitting_grad / self.tau ** 2
        xt_term = (xt - x) / sigma ** 2
        prior_term = self.get_prior_score(x, x0hat, xt, sigma)
        return data_term + xt_term + prior_term, data_fitting_loss

    def get_prior_score(self, x, x0hat, xt, sigma):
    ...
    def prepare_prior_score(self, x0hat, xt, sigma):
        """
    ...
    def mc_prepare(self, x0hat, xt, operator, measurement, sigma):
    ...
    def sample_mh(self, xt, x0hat, operator, measurement, sigma, ratio, record=False, verbose=False):
        if record:
    ...
    def sample(self, xt, x0hat, operator, measurement, sigma, ratio, record=False, verbose=False, task=None):
        """
        ...
        """
        if self.mc_algo == 'mh':
            return self.sample_mh(xt, x0hat, operator, measurement, sigma, ratio, record, verbose)
        if record:
            self.trajectory = Trajectory()
        lr = self.get_lr(ratio)
        self.mc_prepare(x0hat, xt, operator, measurement, sigma)
        self.prepare_prior_score(x0hat, xt, sigma)

        x = x0hat.clone().detach()
        pbar = tqdm.trange(self.num_steps) if verbose else range(self.num_steps)
        for _ in pbar:
            cur_score, fitting_loss = self.score_fn(x, x0hat, xt, operator, measurement, sigma, task)
